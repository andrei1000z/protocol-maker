...
create table if not exists public.daily_metrics (
  ...
  -- Smartwatch / wearable-grade metrics (Galaxy Watch, Oura, WHOOP, Apple, Garmin)
  sleep_hours_planned real,
  sleep_score integer check (sleep_score is null or sleep_score between 0 and 100),
  deep_sleep_min integer,
  light_sleep_min integer,
  rem_sleep_min integer,
  awake_min integer,
  blood_oxygen_avg_sleep real check (blood_oxygen_avg_sleep is null or blood_oxygen_avg_sleep between 70 and 100),
  skin_temp_deviation real,
  hrv_sleep_avg integer,
  bp_systolic_morning integer check (bp_systolic_morning is null or bp_systolic_morning between 60 and 250),
  bp_diastolic_morning integer check (bp_diastolic_morning is null or bp_diastolic_morning between 30 and 160),
  bp_systolic_evening integer check (bp_systolic_evening is null or bp_systolic_evening between 60 and 250),
  bp_diastolic_evening integer check (bp_diastolic_evening is null or bp_diastolic_evening between 30 and 160),
  avg_heart_rate integer check (avg_heart_rate is null or avg_heart_rate between 20 and 220),
  min_heart_rate integer check (min_heart_rate is null or min_heart_rate between 20 and 220),
  max_heart_rate integer check (max_heart_rate is null or max_heart_rate between 20 and 300),
  avg_respiratory_rate real,
  energy_score integer check (energy_score is null or energy_score between 0 and 100),
  active_time_min integer,
  activity_calories integer,
  antioxidant_index integer check (antioxidant_index is null or antioxidant_index between 0 and 100),
  ages_index real,
