...
// Previously we had per-user token trimmers here (trimBryanReference,
// trimInterventionRules) that stripped reference sections not matching the
// user's marker panel. Once prompt caching landed, the trimmers stopped
// helping — the cache pays for the full prefix regardless, and variable-size
// prefixes actually HURT caching because every unique trim is a separate
// cache key. Dropped in favor of a stable CACHEABLE_SYSTEM_PREFIX that hits
// the cache on every call after the first within the 5-min TTL.
...

{
  ...
  "dailySchedule": [
    ...
    "//   1. Wake time",
    "//   2. FASTED MEASUREMENTS — MANDATORY entry 2-5 min after wake, BEFORE food or water.",
    "//      category: 'tracking'. Activity: 'FASTED measurements — before water + food'.",
    "//      Notes must list: weight, body fat %, muscle mass, visceral fat, body water %,",
    "//      basal body temp, antioxidant index (if Veggie Meter), morning BP, resting HR.",
    "//      These are lowest-noise when fasted (water alone shifts BIA body-fat by 2-3%).",
    "//   3. First hydration — 500ml water with pinch of salt + lemon (8-10 min after wake)",
    "//   4. Morning sunlight walk 10 min (anchors circadian)",
    "//   5. Morning supplements — ONE entry PER supplement (list the name + dose inline)",
    "//   6. Breakfast with protein target + recipe hint",
